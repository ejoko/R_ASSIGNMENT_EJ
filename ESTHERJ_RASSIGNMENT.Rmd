...
## Maize
dir.create("Maize data")
## By using the for loop
for (i in 1:10) 
  
## To filter Chromosome "i" data in maize_snp according to the "Chromosome" column
maize_chr <- filter(maize_snp, Chromosome == i)

# To sort generated files based on the "Position column in increasing order

 maize_chr_increasing <-maize_chr[order(maize_chr$Position),]

 ## Create a file to arrange based on increasing position
 maize_increasing <- paste("Maize data/maize_increasing_chr",i,".txt", sep="")
for (i in 1:10){
...
#From the graph, we can detect that chromosome 1 has the most SNPs(~150), whereas chromosome 10 has the least amount of SNPs, (~55)
```
...
ggplot(fang_mt_data) +
  ...
  scale_fill_manual(values = c("blue", "green", "red"))  # Change colors as desired
...
ggplot(fang_mt_data) +
  ...
  scale_fill_manual(values = c("orange", "purple", "yellow"))  # Change colors as desired
...
ggplot(fang_mt_data) +
  ...
  scale_fill_manual(values = c("pink", "brown", "gray"))  # Change colors as desired
...
ggplot(data = fang_mt_own, aes(x = Chromosome, fill = SNP_Data)) +
...
#Across chromosomes, the distribution of heterozygous sites, homozygous sites, and missing data appears to be consistent, with missing data comprising less than 7% and heterozygous sites constituting approximately 13%.
```
